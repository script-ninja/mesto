export default class UserInfo {
  ...
  getUserInfo() {
    // Здравствуйте, Артем!
    // данные устанавливаются через set-методы класса,
    // где они предварительно сохраняются в приватные поля и будут актуальны.
    // Открыв devtools можно руками поменять значения textContent,
    // а так они хотябы в переменных спрятаны и будут соответствовать данным на сервере,
    // поэтому не вижу смысла менять на получение значений из DOM
    return {
      name: this._name,
      info: this._info,
      avatar: this._avatar
